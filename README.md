...
│  │   • Creates rooms & generates 6‑digit codes         │   │
│  │   • Broadcasts messages to all participants         │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  📶 Wi‑Fi Hotspot (enabled on the host device)             │
│     IP: 192.168.43.1 (example)                             │
└─────────────────────────────────────────────────────────────┘
                            │
...
│  3. Start chatting – messages appear on all screens!       │
└─────────────────────────────────────────────────────────────┘

The flow in 3 sentences:

    Host: turns on hotspot → runs node server.js → creates a room → shares the 6‑digit code.

    Guest: connects to the hotspot → opens http://<host-ip>:3000 → enters the code → joins.
